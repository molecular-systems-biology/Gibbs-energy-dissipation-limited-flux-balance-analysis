...
$If not dexist results/Clim $call "mkdir results/Clim"
$If not dexist results/Clim/%gur% $call "mkdir results/Clim/%gur%"

$onecho > ./results/Clim/%gur%/cplex.op3
interactive 1
iatriggerfile results/Clim/%gur%/stopme
iatriggertime 43200
iafile results/Clim/%gur%/cplex.op4
threads 1
parallelmode -1
eprhs 1e-9
epopt 1e-9
...
mipstart 1
memoryemphasis 1
solnpool results/Clim/%gur%/solnpool.gdx
solnpoolreplace 2
solnpoolcapacity 1000
populatelim 1000
solnpoolprefix soln_Clim_%gur%
solnpoolpop 2
$offecho

$onecho > results/Clim/%gur%/cplex.op4
epagap 1e9
$offecho

$echo stopme > results/Clim/%gur%/stopme

$onecho > ./results/Clim/%gur%/conopt.opt
LFNICR 100
LS2NDI 1
RTBND1 1e-9
RTBNDT 1e-10
...
$onecho > ./results/Clim/%gur%/conopt4.opt
LFNICR 100
RTNWMA 1e-9
RTNWMI 5e-11
RTNWTR 1e-9
...
$onecho > ./results/Clim/%gur%/antigone.opt
abs_opt_tol=1e-6
cplex_optfile=/results/Clim/%gur%/cplex.op2
...
$onecho > ./results/Clim/%gur%/cplex.op2
threads 1
epint 1e-9
epopt 1e-9
eprhs 1e-9
...
$GDXIN data/bnds/literature_bnds_rMIP_glucose12.gdx
*$GDXIN data/bnds/fba_rMIPbnds12_defbnds_M9_glucose_ext.gdx
$LOAD V_vgLimits
$LOAD V_drGgLimits
$LOAD V_dfGgLimits
$LOAD V_sigmagLimits
...
** set all amino acid exchange rates to 0
vgLimits(grp,bndID)$(ratio(grp,'EX_ala-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_arg-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_asn-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_asp-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_cys-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_gln-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_glu-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_gly') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_his-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_ile-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_leu-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_lys-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_met-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_phe-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_pro-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_ser-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_thr-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_trp-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_tyr-L') ne 0) = 0;
vgLimits(grp,bndID)$(ratio(grp,'EX_val-L') ne 0) = 0;

** set glucose uptake rate
vgLimits(grp,bndID)$(ratio(grp,'EX_glc') eq 1)=-%gur%;

** include model equations from tcbm.inc
$include data/tcbm_v2bnd.inc

** load and set physiological variable bounds (physiological concentration/drG/drGg bounds were determined from the regression result)
parameter var_drG(rxns,bndID),var_drGg(grp,bndID),var_lnc(mets,comps,bndID);
parameter var_drGLimits(rxns,bndID),var_drGgLimits(grp,bndID),var_lncLimits(mets,comps,bndID);
$GDXIN data/bnds/physiological_bnds.gdx
$LOAD var_drG=drGbnds
$LOAD var_drGg=drGgbnds
$LOAD var_lnc=lncbnds
$GDXIN

var_drGLimits(rxns,'lo') = drGLimits(rxns,'lo');
var_drGLimits(rxns,'up') = drGLimits(rxns,'up');
var_drGgLimits(grp,'lo') = drGgLimits(grp,'lo');
var_drGgLimits(grp,'up') = drGgLimits(grp,'up');
var_lncLimits(mets,comps,'lo') = lncLimits(mets,comps,'lo');
var_lncLimits(mets,comps,'up') = lncLimits(mets,comps,'up');

var_drGLimits(rxns,'lo')$((drGLimits(rxns,'lo') ne drGLimits(rxns,'up')) and (var_drG(rxns,'lo') ne 0) and (var_drG(rxns,'lo') ge drGLimits(rxns,'lo'))) = var_drG(rxns,'lo');
var_drGLimits(rxns,'up')$((drGLimits(rxns,'lo') ne drGLimits(rxns,'up')) and (var_drG(rxns,'up') ne 0) and (var_drG(rxns,'up') le drGLimits(rxns,'up'))) = var_drG(rxns,'up');
var_lncLimits(mets,comps,'lo')$((lncLimits(mets,comps,'lo') ne lncLimits(mets,comps,'up')) and (var_lnc(mets,comps,'lo') ne 0) and (var_lnc(mets,comps,'lo') ge lncLimits(mets,comps,'lo'))) = var_lnc(mets,comps,'lo');
var_lncLimits(mets,comps,'up')$((lncLimits(mets,comps,'lo') ne lncLimits(mets,comps,'up')) and (var_lnc(mets,comps,'up') ne 0) and (var_lnc(mets,comps,'up') ge lncLimits(mets,comps,'up'))) = var_lnc(mets,comps,'up');
var_drGgLimits(grp,'lo')$((drGgLimits(grp,'lo') ne drGgLimits(grp,'up')) and (var_drGg(grp,'lo') ne 0) and (var_drGg(grp,'lo') ge drGgLimits(grp,'lo'))) = var_drGg(grp,'lo');
var_drGgLimits(grp,'up')$((drGgLimits(grp,'lo') ne drGgLimits(grp,'up')) and (var_drGg(grp,'up') ne 0) and (var_drGg(grp,'up') le drGgLimits(grp,'up'))) = var_drGg(grp,'up');

drG.lo(rxns) = var_drGLimits(rxns,'lo');
drG.up(rxns) = var_drGLimits(rxns,'up');
drGg.lo(grp) = var_drGgLimits(grp,'lo');
drGg.up(grp) = var_drGgLimits(grp,'up');
lnc.lo(mets,comps) = var_lncLimits(mets,comps,'lo');
lnc.up(mets,comps) = var_lncLimits(mets,comps,'up');
...
execute_load 'results/Clim/%gur%/solnpool.gdx', solnpool=Index;

loop(solnpool(soln),
  put_utility fsol 'shell' / 'mv ./'solnpool.te(soln):0:0' ./results/Clim/%gur%';
);

** set physiological concentration/drG/drGg bounds
...
loop(solnpool(soln),
  put_utility fsol 'gdxin' / 'results/Clim/%gur%/'solnpool.te(soln):0:0;
  execute_loadpoint;
  ...
  if (((FBA_nlp.modelstat eq 2) or (FBA_nlp.modelstat eq 8) or (FBA_nlp.modelstat eq 1) or (FBA_nlp.modelstat eq 7)),
        put_utility fsol 'gdxout' / 'results/Clim/%gur%/NLP_'solnpool.te(soln):0:0;
...
loop(solnpool(soln),
  put_utility fsol 'shell' / 'rm ./results/Clim/%gur%/'solnpool.te(soln):0:0'';
);

put_utility fsol 'shell' / 'rm ./results/Clim/%gur%/antigone*';
put_utility fsol 'shell' / 'rm ./results/Clim/%gur%/cplex*';
put_utility fsol 'shell' / 'rm ./results/Clim/%gur%/conopt*';
