*** Flux balance analysis with unlimited uptake and various single carbon sources (scond)
*** required input: --scond = x (if not set scond = glucose) ... carbon source
***                 --rep = x (if not set rep = 1) ... iteration

$If NOT set scond $set scond glucose
...
** allow the uptake of the specified carbon source
set cond/acetate,fructose,galactose,gluconate,glucose,glycerol,pyruvate,succinate,glucose+succinate/,runcond(cond);
runcond(cond)=no;
runcond('%scond%')=yes;
loop(cond$runcond(cond),
         if(sameas(cond,'acetate'),vgLimits(grp,'up')$(ratio(grp,'EX_ac') eq -1)=500);
         if(sameas(cond,'fructose'),vgLimits(grp,'lo')$(ratio(grp,'EX_fru') eq 1)=-500);
         if(sameas(cond,'galactose'),vgLimits(grp,'lo')$(ratio(grp,'EX_gal') eq 1)=-500);
         if(sameas(cond,'gluconate'),vgLimits(grp,'lo')$(ratio(grp,'EX_glcn') eq 1)=-500);
         if(sameas(cond,'glucose'),vgLimits(grp,'lo')$(ratio(grp,'EX_glc') eq 1)=-500);
         if(sameas(cond,'glycerol'),vgLimits(grp,'lo')$(ratio(grp,'EX_glyc') eq 1)=-500);
         if(sameas(cond,'pyruvate'),vgLimits(grp,'up')$(ratio(grp,'EX_pyr') eq -1)=500);
         if(sameas(cond,'succinate'),vgLimits(grp,'lo')$(ratio(grp,'EX_succ') eq 1)=-500);
         if(sameas(cond,'glucose+succinate'),vgLimits(grp,'lo')$(ratio(grp,'EX_glc') eq 1)=-500;
                                             vgLimits(grp,'lo')$(ratio(grp,'EX_succ') eq 1)=-500);
);
